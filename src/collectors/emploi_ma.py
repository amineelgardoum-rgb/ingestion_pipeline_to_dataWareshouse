    ...
    for card in cards:
        try:
            ...
            for li in card.select("ul li"):
                ...
                elif "Contrat" in text:
                    contract = strong.get_text(strip=True)
...
def _scrape_term(term: str, seen_urls: set, lock: Lock, max_pages: int = MAX_PAGES) -> list[dict]:
    ...
    jobs     = []
    base_url = SEARCH_URL.format(term=quote(term, safe=""))
...
def collect(max_pages: int = MAX_PAGES, max_workers: int = MAX_WORKERS) -> pd.DataFrame:
    logger.info("=" * 50)
    logger.info(f"Starting Emploi.ma — {len(settings.SEARCH_TERMS)} terms, {max_workers} workers")
    logger.info("=" * 50)
...
if __name__ == "__main__":
    df = collect()
    print(f"\nShape: {df.shape}")
    print(df[["title", "company", "location", "contract", "date_posted"]].head(10))